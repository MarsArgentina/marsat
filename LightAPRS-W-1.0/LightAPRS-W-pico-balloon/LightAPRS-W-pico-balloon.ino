...
char    comment[50] = "Esto es una prueba - TMSA ar"; // Max 50 char
char    StatusMessage[50] = "Esto es una prueba - TMSA ar";
//*****************************************************************************
...
void setup() {
  ...
  AprsPinInput;
  bmp.begin();
  Wire.begin(4); //unirse a un bus i2c, se une con el identificador 4
...
String wireTemp;
void receiveEvent(int howMany)//funcion que se ejcuta cuando llega un mensaje por i2c
{
...
void gpsDebug() {
  ...
  Serial.println();
  //sólo podemos transmitir 32 bytes en cada tramisión de i2c
  Wire.beginTransmission(4); // transmit to device #4
  Wire.write("start");
  Wire.write("lat: ");        
  Wire.write(String(gps.location.lat(),6);              
  Wire.endTransmission();
  Wire.beginTransmission(4); // transmit to device #4
  Wire.write("long: ");        
  Wire.write(String(gps.location.lng(),6);              
  Wire.endTransmission();     
  Wire.beginTransmission(4); // transmit to device #4
  Wire.write("alt: ");        
  Wire.write(String(gps.altitude.meters(),6);      
  Wire.write("stop");        
  Wire.endTransmission(); 
#endif
}
